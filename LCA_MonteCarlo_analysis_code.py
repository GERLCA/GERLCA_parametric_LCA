...
#Python 3 code to run a Monte Carlo analysis for a life cycle assessment (LCA) model, for one impact category.
...
for n in range(1,(MonteCarlo_runs+1)):
    ...
    for pname,minimum,baseline,maximum,distributiontype in prange:
        ...
        if distributiontype=="lognormal\n" or distributiontype=="lognormal":
            ...
            #for lognormvariate, the location is the baseline and the scale is the maximum, from the Minitab lognormal distribution fit data.
            while pvalue<minimum:
                pvalue=lognormvariate(baseline,maximum)
