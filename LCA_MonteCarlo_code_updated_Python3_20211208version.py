...
#Date: 7/22/2014 to 7/24/2014, modified on 3/29/2016, modified on 4/25/2018, 7/11/2018, and 9-10/2018 through 2019 and 2020; weighed distributions fixed on 12/8/2021
#Monte Carlo in Python for LCA purposes
#Works for a comma-delimited text file of parameter inputs and ranges, and a list of equations
#Works with if-then statements as entered into Excel.
#Make sure that variables appear in order of definition in the equations list.
...
print('This program will produce a Monte Carlo analysis for a life cycle assessment with different probability distributions of parameter inputs.')
...
print('The equations for the entire LCA should be organized in order that new variables appear in a one-column text file without a header.')
print('')
print('For parameters with raw data to define their probability distribution, label these as "weighed" for distribution type.')
...
for n in range(1,(MonteCarlo_runs+1)):
    ...
    for pname,minimum,baseline,maximum,distributiontype in prange:
        if distributiontype=="weighed\n" or distributiontype=="weighed":
            ...
            pvalue=choices(weighedparametervalue,weights=weighedprob,k=1)
            pvalue=pvalue[0]
    ...
    MonteCarlo_outputlist.append(Total_Impact)
    parameterdict.clear()
    print(str(n)+'...')

MonteCarlo_outputs.writelines('\n'.join(map(str,MonteCarlo_outputlist)))
